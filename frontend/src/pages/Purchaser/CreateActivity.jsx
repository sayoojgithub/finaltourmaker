...
const CreateActivity = () => {
  ...
  const fetchStates = async (countryId) => {
    try {
      ...
      setStates(res.data); // Assuming you have states state
    } catch (err) {
  ...
  // useEffect just calls it with formData
  useEffect(() => {
    ...
    try {
      if (countryId && stateId && destinationId) {
        const [vendorsRes, tripsRes] = await Promise.all([
          API.get(
            `/purchaser/vendorsOfActivities/${countryId}/${stateId}/${destinationId}`
          ),
          API.get(
  ...
  const handleFieldChange = (index, field, value) => {
    ...
    updated[index].itineraryPrice = isNaN(itineraryPrice)
      ? ""
      : itineraryPrice.toFixed(2);
  ...
  const handleEdit = async (activity) => {
    try {
      ...
      // 1. Fetch states based on selected country
      await fetchStates(activity.country?._id);

      // 2. Fetch destinations based on selected country and state
      await fetchDestinations(activity.country?._id, activity.state?._id);

      // 3. Fetch vendors and trips based on selected country, state, and destination
      await fetchData(
        activity.country?._id,
        activity.state?._id,
        activity.destination?._id
      );

      // 4. Now set the form data and price fields
      setFormData({
  ...
  const handleCreateActivity = async () => {
    ...
    for (let i = 0; i < priceFields.length; i++) {
      ...
      if (!from || !to || !price) {
        toast.error(
          `Price row ${i + 1} is incomplete. Please fill From, To, and Price.`
        );
        return;
      }

      // Optional: validate date order and price value
      if (new Date(from) >= new Date(to)) {
        toast.error(
          `Price row ${i + 1}: 'From' date must be before 'To' date.`
        );
        return;
      }
    ...
    const parsedRanges = priceFields.map((p) => ({
    ...
    for (let i = 0; i < parsedRanges.length; i++) {
      for (let j = i + 1; j < parsedRanges.length; j++) {
        ...
        if (
          a.from <= b.to &&
          a.to >= b.from // Overlapping condition
        ) {
          toast.error(
            `Date ranges in price row ${i + 1} and ${j + 1} are overlapping.`
          );
          return;
        }
      }
    ...
    try {
      const payload = {
        ...
        prices: priceFields.map((p) => ({
          ...
          itineraryPrice: Number(p.itineraryPrice),
        })),
        imageUrl,
      };
      ...
      if (editingActivityId) {
        ...
        const res = await API.put(
          `/purchaser/updateActivity/${editingActivityId}`,
          payload
        );
        toast.success("Activity updated successfully!");
      } else {
        // CREATE flow
        const res = await API.post("/purchaser/createActivity", payload);
      ...
      toast.error(
        `Error creating activity: ${err.response?.data?.error || err.message}`
      );
    }
  };
  ...
  const handleToggleStatus = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        toast.success(
          `Activity ${updatedStatus ? "activated" : "deactivated"} successfully`
        );
        await fetchActivities(); // refresh table
      } else {
      ...
      <div className="space-y-6">
        <div>
          ...
          <select
            ...
            value={formData.trip}
            onChange={(e) =>
              setFormData((prev) => ({ ...prev, trip: e.target.value }))
            }
          >
          ...
          {priceFields.map((field, index) => (
            ...
            >
              <input
                ...
                value={field.from}
                onChange={(e) =>
                  handleFieldChange(index, "from", e.target.value)
                }
              />
              <input
                type="date"
