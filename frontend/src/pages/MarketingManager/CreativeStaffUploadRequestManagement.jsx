// src/pages/marketing/CreativeStaffUploadRequestManagement.jsx
import React, { useEffect, useMemo, useState } from "react";
import API from "../../api";
import Select from "react-select";
import { toast } from "react-toastify";

export default function CreativeStaffUploadRequestManagement() {
  // table state
  ...
  const buildQuery = (nextPage = page) => {
    ...
    params.set("limit", "10");
    if (filterCategoryText.trim()) params.set("categoryText", filterCategoryText.trim());
    ...
    if (filterStatus?.value) params.set("status", filterStatus.value); // pending|waiting|approved|rejected
  ...
  const fetchList = async (nextPage = page) => {
    try {
      setLoading(true);
      const res = await API.get(`/marketingManager/creative-upload-requests?${buildQuery(nextPage)}`);
  ...
  useEffect(() => {
    fetchList(1);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [filterCategoryText, filterFilenameText, filterCreativeStaffText, filterStatus, filterPublishingDate]);
  ...
  const openPanel = async (id) => {
    ...
    setRejectReason("");
    try {
      const res = await API.get(`/marketingManager/creative-upload-requests/${id}`);
      setDetail(res.data);
  ...
  const approve = async () => {
    ...
    try {
      await API.post(`/marketingManager/creative-upload-requests/${detail._id}/approve`);
  ...
  const reject = async () => {
    ...
    try {
      await API.post(`/marketingManager/creative-upload-requests/${detail._id}/reject`, {
  ...
  return (
    <div className="space-y-6">
      {/* <h2 className="text-xl font-semibold text-[#222]">Creative Staff — Upload Requests (Company-wide)</h2> */}
      ...
      {openId && (
        <div className="rounded-2xl border border-gray-200 p-5 shadow-sm bg-white">
          ...
          {panelLoading || !detail ? (
            ...
            <>
             {/* Row 1: Category / File Name / Publishing Date */}
<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
  ...
  <Field label="Publishing Date">
    <input
      ...
      readOnly
      className="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 text-gray-800"
      value={formatDMY(detail.approvedPublishingDate || detail.publishingDate)}
    />
  </Field>
</div>

{/* Row 2: Creative Staff / Creative Status */}
...
{/* Row 3: Message for Creative Staff (full width) */}
<div className="mt-4">
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="divide-y divide-gray-100 bg-white">
            {loading ? (
              ...
              rows.map((r) => {
                const pub = r.approvedPublishingDate || r.publishingDate;
