// src/pages/marketing/CreativeStaffUploadTaskManagement.jsx
import React, { useEffect, useMemo, useState } from "react";
import API from "../../api";
import Select from "react-select";
import { toast } from "react-toastify";

export default function CreativeStaffUploadTaskManagement() {
  // table state
  ...
  const fetchList = async (nextPage = page) => {
    try {
      setLoading(true);
      const res = await API.get(`/marketingManager/creative-upload-assignments?${buildQuery(nextPage)}`);
      ...
      toast.error("Failed to load upload tasks");
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    fetchList(1);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [filterCategoryText, filterFilenameText, filterCreativeStaffText, filterPublishingDate, filterStatus]);
  ...
  const openPanel = async (id) => {
    ...
    setRejectReason("");
    try {
      const res = await API.get(`/marketingManager/creative-upload-assignments/${id}`);
      setDetail(res.data);
  ...
  const approve = async () => {
    ...
    try {
      await API.post(`/marketingManager/creative-upload-assignments/${detail._id}/approve`);
  ...
  const reject = async () => {
    ...
    try {
      await API.post(`/marketingManager/creative-upload-assignments/${detail._id}/reject`, {
      ...
      {openId && (
        <div className="rounded-2xl border border-gray-200 p-5 shadow-sm bg-white">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold">Review Upload Task — Creative Status</h3>
            <button
              onClick={closePanel}
          ...
          {panelLoading || !detail ? (
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <Field label="Publishing Date">
                  <input
                    readOnly
                    className="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2"
                    value={dmy(detail.publishingDate)}
                  />
                </Field>
              </div>

              {/* Row 2: Creative Staff / Status */}
              ...
              {/* Row 3: Message for Creative Staff */}
              <div className="mt-4">
