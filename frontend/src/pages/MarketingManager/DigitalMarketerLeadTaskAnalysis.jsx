// src/components/DigitalMarketerLeadTaskAnalysis/DigitalMarketerLeadTaskAnalysis.jsx
...
export default function DigitalMarketerLeadTaskAnalysis() {
  ...
  const [campaignText, setCampaignText] = useState(""); // matches campaignName
  const [frequency, setFrequency] = useState(""); // daily|weekly|monthly
  const [startDate, setStartDate] = useState(""); // yyyy-mm-dd (match LeadAssignment.startDate)
  ...
  const buildQuery = (nextPage = page) => {
    ...
    if (campaignText.trim()) params.set("campaignText", campaignText.trim());
  ...
  const fetchList = async (nextPage = page) => {
    try {
      setLoading(true);
      const res = await API.get(`/marketingManager/dm-lead-tasks?${buildQuery(nextPage)}`);
  ...
  }, [destinationText, campaignText, frequency, startDate, posted]);
