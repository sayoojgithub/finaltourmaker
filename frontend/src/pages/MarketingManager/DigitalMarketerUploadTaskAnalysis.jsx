...
export default function DigitalMarketerUploadTaskAnalysis() {
  ...
  const fetchList = async (nextPage = page) => {
    try {
      setLoading(true);
      const res = await API.get(`/marketingManager/dm-upload-assignments?${buildQuery(nextPage)}`);
            ...
            {loading ? (
              ...
              tableData.map((row) => (
                <tr key={row.id} className="hover:bg-gray-50">
                  ...
                  <Td className={row.creativeStatus === "approved" ? "text-green-700" : row.creativeStatus === "rejected" ? "text-red-600" : "text-amber-700"}>
                    {row.creativeStatus}
                  </Td>
