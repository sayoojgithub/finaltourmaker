// src/pages/marketing/CreativeStaffAssignedAdTaskManagement.jsx
import React, { useEffect, useMemo, useState } from "react";
import API from "../../api";
import Select from "react-select";
import { toast } from "react-toastify";

export default function CreativeStaffAssignedAdTaskManagement() {
  // table state
  ...
  const fetchList = async (nextPage = page) => {
    try {
      setLoading(true);
      const res = await API.get(`/marketingManager/creative-ad-assignments?${buildQuery(nextPage)}`);
      ...
      toast.error("Failed to load creative ad assignments");
    } finally {
      setLoading(false);
    }
  };
  ...
  const openPanel = async (id) => {
    ...
    setRejectReason("");
    try {
      const res = await API.get(`/marketingManager/creative-ad-assignments/${id}`);
      setDetail(res.data);
  ...
  const approve = async () => {
    ...
    try {
      await API.post(`/marketingManager/creative-ad-assignments/${detail._id}/approve`);
  ...
  const reject = async () => {
    ...
    try {
      await API.post(`/marketingManager/creative-ad-assignments/${detail._id}/reject`, {
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-xl font-semibold text-[#222]">Creative Staff — Assigned Ad Tasks (Company-wide)</h2>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="divide-y divide-gray-100 bg-white">
            {loading ? (
              ...
              rows.map((r) => {
                const deadline = r.date;
                const requiredQty = r.quantity ?? "—";
