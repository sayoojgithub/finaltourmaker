// src/pages/marketing/CreativeStaffAssignedLeadTaskManagement.jsx
import React, { useEffect, useMemo, useState } from "react";
import API from "../../api";
import Select from "react-select";
import { toast } from "react-toastify";

export default function CreativeStaffAssignedLeadTaskManagement() {
  // table state
  ...
  const fetchList = async (nextPage = page) => {
    try {
      setLoading(true);
      const res = await API.get(`/marketingManager/creative-assigned-leads?${buildQuery(nextPage)}`);
      ...
      toast.error("Failed to load assigned creative tasks");
    } finally {
      setLoading(false);
    }
  };
  ...
  const openPanel = async (id) => {
    ...
    try {
      const res = await API.get(`/marketingManager/creative-assigned-leads/${id}`);
      setDetail(res.data);
  ...
  const approve = async () => {
    ...
    try {
      await API.post(`/marketingManager/creative-assigned-leads/${detail._id}/approve`);
  ...
  const reject = async () => {
    ...
    try {
      await API.post(`/marketingManager/creative-assigned-leads/${detail._id}/reject`, {
      ...
      {openId && (
        <div className="rounded-2xl border border-gray-200 p-5 shadow-sm bg-white">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold">Review Assigned Task — Creative Status</h3>
            <button
              onClick={closePanel}
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <Field label="Start Date">
                  <input readOnly className="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2" value={dmy(detail.startDate)} />
                </Field>
              </div>

              {/* Row 2: End / Qty / Frequency */}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                <Field label="End Date">
                  <input readOnly className="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2" value={dmy(detail.endDate)} />
                </Field>
                <Field label="Quantity">
                  <input readOnly className="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2" value={detail.quantity ?? "—"} />
                </Field>
                <Field label="Frequency">
                  <input readOnly className="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2" value={detail.frequency || "—"} />
                </Field>
              </div>

              {/* Row 3: Creative Staff & Status */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                ...
                <Field label="Current Creative Status">
                  <input readOnly className="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2" value={detail.creativeStatus} />
                </Field>
              </div>
              ...
              <div className="mt-4">
                ...
                {detail.fileNames?.length ? (
                  ...
                  <p className="textSm text-gray-600">No files listed.</p>
                )}
              </div>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="divide-y divide-gray-100 bg-white">
            {loading ? (
              ...
              <tr>
                <td className="px-6 py-4 text-sm text-gray-500" colSpan={9}>No tasks.</td>
              </tr>
            ) : (
              rows.map((r) => {
