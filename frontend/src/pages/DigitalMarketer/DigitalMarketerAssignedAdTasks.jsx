// src/pages/digital/DigitalMarketerAssignedAdTasks.jsx
import React, { useEffect, useMemo, useState } from "react";
import API from "../../api";
import Select from "react-select";
import { toast } from "react-toastify";

export default function DigitalMarketerAssignedAdTasks() {
  // table state
  ...
  const fetchList = async (nextPage = page) => {
    try {
      setLoading(true);
      const res = await API.get(`/digitalMarketer/assigned-ad-tasks?${buildQuery(nextPage)}`);
      ...
      toast.error("Failed to load assigned ad tasks");
    } finally {
      setLoading(false);
    }
  };
  ...
  const openPanel = async (id) => {
    ...
    setDetail(null);
    try {
      const res = await API.get(`/digitalMarketer/assigned-ad-tasks/${id}`);
      setDetail(res.data);
    } catch {
  ...
  const markDone = async () => {
    ...
    try {
      await API.post(`/digitalMarketer/assigned-ad-tasks/${detail._id}/mark-posted`);
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-xl font-semibold text-[#222]">Digital Marketer — Assigned Ad Tasks</h2>

      {/* ---------- INLINE PANEL ---------- */}
      {openId && (
        <div className="rounded-2xl border border-gray-200 p-5 shadow-sm bg-white">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold">Assigned Ad Task</h3>
            <button
              onClick={closePanel}
