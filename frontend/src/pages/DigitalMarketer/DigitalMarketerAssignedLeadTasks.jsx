// src/pages/digital/DigitalMarketerAssignedLeadTasks.jsx
import React, { useEffect, useMemo, useState } from "react";
import API from "../../api";
import Select from "react-select";
import { toast } from "react-toastify";

export default function DigitalMarketerAssignedLeadTasks() {
  // table state
  ...
  const fetchList = async (nextPage = page) => {
    try {
      setLoading(true);
      const res = await API.get(`/digitalMarketer/assigned-lead-tasks?${buildQuery(nextPage)}`);
      ...
      toast.error("Failed to load assigned lead tasks");
    } finally {
      setLoading(false);
    }
  };
  ...
  const openPanel = async (id) => {
    ...
    setDetail(null);
    try {
      const res = await API.get(`/digitalMarketer/assigned-lead-tasks/${id}`);
      setDetail(res.data);
    } catch {
  ...
  const markDone = async () => {
    ...
    try {
      await API.post(`/digitalMarketer/assigned-lead-tasks/${detail._id}/mark-posted`);
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-xl font-semibold text-[#222]">Digital Marketer — Assigned Lead Tasks</h2>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="divide-y divide-gray-100 bg-white">
            {loading ? (
              ...
              <tr><td className="px-6 py-4 text-sm text-gray-500" colSpan={9}>No tasks.</td></tr>
            ) : (
              rows.map((r) => {
