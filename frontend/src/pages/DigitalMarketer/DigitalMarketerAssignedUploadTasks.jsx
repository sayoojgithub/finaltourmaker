// src/pages/digital/DigitalMarketerAssignedUploadTasks.jsx
import React, { useEffect, useMemo, useState } from "react";
import API from "../../api";
import Select from "react-select";
import { toast } from "react-toastify";

export default function DigitalMarketerAssignedUploadTasks() {
  // table state
  ...
  const fetchList = async (nextPage = page) => {
    try {
      setLoading(true);
      const res = await API.get(`/digitalMarketer/upload-assignments?${buildQuery(nextPage)}`);
      ...
      toast.error("Failed to load assigned upload tasks");
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => { fetchList(1); /* eslint-disable-next-line */ },
    [filterCategoryText, filterFilenameText, filterPublishingDate, filterPostStatus]
  );
  ...
  const openPanel = async (id) => {
    ...
    setDetail(null);
    try {
      const res = await API.get(`/digitalMarketer/upload-assignments/${id}`);
      setDetail(res.data);
    } catch {
  ...
  const markDone = async () => {
    ...
    try {
      await API.post(`/digitalMarketer/upload-assignments/${detail._id}/mark-posted`);
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-xl font-semibold text-[#222]">Digital Marketer — Assigned Upload Tasks</h2>

      {/* ---------- INLINE PANEL ---------- */}
      {openId && (
        <div className="rounded-2xl border border-gray-200 p-5 shadow-sm bg-white">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold">Assigned Upload Task</h3>
            <button
              onClick={closePanel}
          ...
          {panelLoading || !detail ? (
            ...
            <>
              ...
              {/* Row 2: Message for DM (FULL WIDTH) */}
              <div className="mt-4">
      ...
      <div className="overflow-x-auto rounded-2xl border border-gray-200">
        ...
        <tbody className="divide-y divide-gray-100 bg-white">
          {loading ? (
            ...
            <tr>
              <td className="px-6 py-4 text-sm text-gray-500" colSpan={6}>No tasks.</td>
            </tr>
          ) : (
            rows.map((r) => {
              ...
              return (
                <tr key={r._id} className="hover:bg-gray-50">
                  <Td>{r.category || "—"}</Td>
                  <Td>{r.filename || "—"}</Td>
                  <Td>{formatDMY(r.publishingDate)}</Td>
                  <Td>
