// src/pages/digital/DigitalMarketerUploadRequests.jsx
import React, { useEffect, useMemo, useState } from "react";
import API from "../../api";
import Select from "react-select";
import { toast } from "react-toastify";

export default function DigitalMarketerUploadRequests() {
  ...
  const fetchList = async (nextPage = page) => {
    try {
      setLoading(true);
      const res = await API.get(`/digitalMarketer/upload-requests?${buildQuery(nextPage)}`);
  ...
  const openPanel = async (id) => {
    ...
    setDetail(null);
    try {
      const res = await API.get(`/digitalMarketer/upload-requests/${id}`);
      setDetail(res.data);
    } catch {
  ...
  const markDone = async () => {
    ...
    try {
      await API.post(`/digitalMarketer/upload-requests/${detail._id}/mark-posted`);
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-xl font-semibold text-[#222]">Digital Marketer — Upload Requests</h2>

      {/* ---------- INLINE PANEL (ABOVE filters & table) ---------- */}
      {openId && (
        <div className="rounded-2xl border border-gray-200 p-5 shadow-sm bg-white">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold">Upload Request</h3>
            <button
              onClick={closePanel}
          ...
          {panelLoading || !detail ? (
            ...
            <>
              ...
              {/* Files (BELOW message, FULL WIDTH) */}
              <div className="mt-4">
                <Field label="Files">
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="divide-y divide-gray-100 bg-white">
            {loading ? (
              ...
              <tr>
                <td className="px-6 py-4 text-sm text-gray-500" colSpan={6}>No requests.</td>
              </tr>
            ) : (
              rows.map((r) => {
