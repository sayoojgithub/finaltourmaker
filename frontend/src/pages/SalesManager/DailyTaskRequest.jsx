...
export default function DailyTaskRequest() {
  ...
  const [dateInput, setDateInput] = useState("");       // single picker value
  const [dates, setDates] = useState([]);               // selected dates array (YYYY-MM-DD)
  ...
  // Decision (approved / rejected / processing) info (shown only for prefilled rows)
  ...
  const [filterDate, setFilterDate] = useState(""); // single-date filter
  ...
  // Lock the form when prefilled (absolutely nothing editable)
  ...
  // Dedup/Sort helper for dates
  const normalizeDates = (arr) =>
    Array.from(new Set(arr.filter(Boolean))).sort((a, b) => new Date(a) - new Date(b));
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    // never submit while prefilled/locked
    if (isLocked) return;

    if (!dates.length) {
      toast.error("Please add at least one target date.");
      return;
    }
    ...
    try {
      ...
      await API.post("/salesManager/daily-task-requests", payload);
      toast.success("Daily task request submitted successfully!");
      clearPrefillAndForm();
      setPage(1);
  ...
  const fetchRequests = async (
    ...
    try {
      ...
      const res = await API.get(`/salesManager/daily-task-requests?${params.toString()}`);
  ...
  // Prefill (from table "+"). After prefill, absolutely nothing editable.
  const handlePrefill = async (id) => {
    try {
      ...
      const res = await API.get(`/salesManager/daily-task-requests/${id}`);
      const r = res.data;
      if (!r || !r._id) {
        setPrefillActive(false);
        setChangeSource(null);
      ...
      // Multi-dates (support both shapes)
      const incomingDates = Array.isArray(r.dates) ? r.dates : r.date ? [r.date] : [];
      const normalizedRequested = normalizeDates(incomingDates.map((d) => (d ? String(d).slice(0, 10) : d)));
      setDates(normalizedRequested);
      setDateInput("");
      ...
      const normalizedApproved = Array.isArray(r.approvedDates)
        ? normalizeDates(r.approvedDates.map((d) => (d ? String(d).slice(0, 10) : d)))
        : [];

      setDecisionMeta({
        status: r.status, // 'approved' | 'rejected' | 'processing'
        requestedDates: normalizedRequested,          // array of 'YYYY-MM-DD'
        requestedQuantity: r.quantity ?? null,
        approvedDates: normalizedApproved,            // array or []
        approvedQuantity: r.approvedQuantity ?? null, // number or null
  ...
  // When user changes anything, mark source as 'user' (won't happen while locked)
  const userChange = () => {
    setPrefillId(null);
  ...
  const addDate = () => {
    if (!dateInput || isLocked) return; // guard while locked
    const next = normalizeDates([...dates, dateInput]);
    if (next.length === dates.length) {
      toast.info("That date is already added.");
    }
    setDates(next);
    setDateInput("");
    userChange();
  ...
  // Remove a date from the list (DISABLED while prefilled)
  const removeDate = (d) => {
    if (isLocked) return;
  ...
  return (
    <div className="space-y-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Field label="Destination" required>
            <Select
              ...
              styles={selectStyles}
              classNamePrefix="dtreq-destination"
            />
          </Field>
          ...
          <Field label="Task" required>
            <Select
              ...
              styles={selectStyles}
              classNamePrefix="dtreq-task"
            />
          </Field>
        </div>

        {/* Row 2: Target Dates / Quantity */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-start">
          {/* Left: Target Dates */}
          <Field label="Target Dates" required>
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                <input
                  type="date"
                  disabled={isLocked}
                  className="flex-1 rounded-xl border border-gray-300 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#8570EE] disabled:opacity-100 disabled:bg-white disabled:text-gray-900 disabled:cursor-not-allowed"
                  value={dateInput}
              ...
              {/* Selected dates as chips (delete disabled while locked) */}
              {dates.length > 0 ? (
                <ul className="flex flex-wrap gap-2">
                  {dates.map((d) => (
                    ...
                    >
                      ...
                      <span className="text-gray-600">({weekdayName(d)})</span>
                      <button
                        type="button"
                        onMouseDown={(e) => e.preventDefault()}
                        onClick={() => removeDate(d)}
                        disabled={isLocked}
                        className="ml-1 inline-flex h-6 w-6 items-center justify-center rounded-full border border-gray-300 hover:bg-gray-200 text-gray-700 disabled:opacity-50"
                        title="Remove date"
                      >
                        ×
                      </button>
      ...
      <section className="space-y-4">
        <h3 className="text-lg font-semibold text-[#222]">My Daily Task Requests</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Field label="Filter by Destination">
            <Select
              ...
              styles={selectStyles}
              classNamePrefix="dtreq-filter-destination"
            />
          </Field>
          ...
          <Field label="Filter by Task">
            <Select
              ...
              styles={selectStyles}
              classNamePrefix="dtreq-filter-task"
            />
          </Field>

          <Field label="Filter by Target Date">
            <input
              type="date"
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="divide-y divide-gray-100 bg-white">
              {loadingTable ? (
                ...
                rows.map((r) => {
                  const rowDates = Array.isArray(r.dates)
                    ? r.dates
                    : r.date
                    ? [r.date]
                    : [];
                  const pretty = normalizeDates(
                    rowDates.map((d) => (d ? String(d).slice(0, 10) : d))
                  )
                    .map((d) => `${formatDMY(d)} (${weekdayName(d)})`)
                    .join(", ");
...
function DecisionPanel({ meta, formatDMY, weekdayName }) {
  const status = meta?.status || "processing";

  // Effective = approved override if present, else requested
  const effectiveDates =
    Array.isArray(meta?.approvedDates) && meta.approvedDates.length > 0
      ? meta.approvedDates
      : Array.isArray(meta?.requestedDates)
      ? meta.requestedDates
      : [];
  const sameDates = !(Array.isArray(meta?.approvedDates) && meta.approvedDates.length > 0);

  const effectiveQuantity =
    meta?.approvedQuantity != null ? meta.approvedQuantity : meta?.requestedQuantity ?? "—";

  const prettyDates = effectiveDates.map((d) => {
    const iso = d ? String(d).slice(0, 10) : d;
    return `${formatDMY(iso)} (${weekdayName(iso)})`;
  });
  ...
  const GreenInfo = ({ label, children, same }) => (
    <div className="rounded-xl border border-green-200 bg-green-50 p-3">
      <div className="text-xs font-medium text-green-700">{label}</div>
      <div className="mt-1 text-sm font-semibold text-green-900">{children}</div>
    ...
    <div className="rounded-2xl border border-gray-200 p-4">
      ...
      {/* {status === "approved" && (
        <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
          <GreenInfo label="Approved Dates" same={sameDates}>
            {prettyDates.length ? (
              // INLINE version (comma-separated, wraps naturally)
              <span className="whitespace-normal">{prettyDates.join(", ")}</span>
            ) : (
              "—"
            )}
          </GreenInfo>
          <GreenInfo label="Approved Quantity" same={meta?.approvedQuantity == null}>
            {effectiveQuantity}
          </GreenInfo>
        </div>
      )} */}
  ...
  <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
    ...
    {/* NEW: Updation Reason (full width) */}
    {meta?.updationReason && (
      ...
      {status === "processing" && (
        <div className="mt-3 text-sm text-gray-600">
          Your request is under review. Requested Dates:{" "}
          {prettyDates.length ? (
            <span className="font-medium text-gray-900">{prettyDates.join(", ")}</span>
          ) : (
            <span className="font-medium text-gray-900">—</span>
          )}{" "}
          • Quantity: <span className="font-medium text-gray-900">{effectiveQuantity}</span>
        </div>
      )}
    </div>
  );
