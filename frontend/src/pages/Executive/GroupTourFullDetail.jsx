...
// function asTextList(arr, pick) {
//   if (!Array.isArray(arr) || arr.length === 0) return [];
//   return arr
//     .map((x) => {
//       if (!x) return null;
//       if (typeof x === "string") return x;
//       if (typeof x === "object" && pick && x[pick]) return x[pick];
//       if (typeof x === "object" && x.name) return x.name;
//       if (typeof x === "object" && x.label) return x.label;
//       return null;
//     })
//     .filter(Boolean);
// }
...
//   Main Component
// ------------------------------ */
// export default function GroupTourFullDetail({
//   tour, // { clientId, id, name?, destination?, totalDays?, seatsAvailable?, articleNumber? }
//   brandColor,
//   onClose,
//   onCompleted,
// }) {
...
//   // ✅ fetch tour+client details when modal opens OR tour changes
//   useEffect(() => {
...
//         const res = await API.get("/executive/group-tour-preview", {
//           params: { clientId: tour.clientId, groupTourId: tour.id },
//         });
...
//           err.message ||
//           "Failed to load group tour details";
//         toast.error(msg);
//       } finally {
...
//   const tourName = tourFull?.tourName || tour?.name || "Group Tour";
//   const destName = nameOrDash(tourFull?.destination) || tour?.destination || "-";
//   const totalDays = tourFull?.totalDays ?? tour?.totalDays ?? "-";
//   const seatsAvail = tourFull?.seatsAvailable ?? tour?.seatsAvailable ?? "-";
...
//   const totalSteps = 1 + (days.length || 0); // overview + days

//   // If stepIndex > available (rare), clamp
//   const safeStepIndex = Math.min(
//     Math.max(0, stepIndex),
//     Math.max(0, totalSteps - 1)
//   );

//   // current day index in days[] (only valid when safeStepIndex >= 1)
//   const dayIdx = safeStepIndex - 1;
//   const day = dayIdx >= 0 ? days[dayIdx] : null;

//   // ✅ build clean text-only view model for current day (NO vendor/price/qty)
...
//       // ✅ Vehicles: prefer vehicleName then category
//       const tripVehicles = Array.isArray(seg?.boTripVehicles)
//         ? seg.boTripVehicles
//             .map((v) => v?.vehicleName || v?.category)
//             .filter(Boolean)
//         : [];

//       const addonVehicles = Array.isArray(seg?.boAddonVehicles)
//         ? seg.boAddonVehicles
//             .map((v) => v?.vehicleName || v?.category)
//             .filter(Boolean)
//         : [];

//       // ✅ Food: show mealType + foodName pair
//       const foods = Array.isArray(seg?.boFoods)
//         ? seg.boFoods
//             .map((f) => {
//               const mt = f?.mealType || "";
//               const fn = f?.foodName || "";
//               const out = `${mt}${mt && fn ? " - " : ""}${fn}`.trim();
//               return out || null;
//             })
//             .filter(Boolean)
//         : [];
...
//       const stays = Array.isArray(seg?.boAccommodations)
//         ? seg.boAccommodations
//             .map((a) => {
//               const pn = a?.propertyName || "";
//               const hc = a?.hotelCategory ? ` (${a.hotelCategory})` : "";
//               const rc = a?.roomCategory ? ` - ${a.roomCategory}` : "";
//               const rt = a?.roomTypeCode ? ` [${a.roomTypeCode}]` : "";
//               const out = `${pn}${hc}${rc}${rt}`.trim();
//               return out || null;
//             })
//             .filter(Boolean)
//         : [];
...
//       dateText: day.date ? formatDate(day.date) : null,
//       segmentBlocks,
//     };
//   }, [day, dayIdx]);
...
//       await API.post("/executive/group-tour-referral-itinerary", {
//         clientId: tour.clientId,
//         groupTourId: tour.id,
//         groupTourName: tourFull?.tourName || tour?.name,
...
//       await API.post("/executive/group-tour-confirm-itinerary", {
//         clientId: tour.clientId,
//         groupTourId: tour.id,
//         groupTourName: tourFull?.tourName || tour?.name,
...
//                                   <GlassChip theme={theme}>Category: {overview.category}</GlassChip>
//                                   <GlassChip theme={theme}>Start: {overview.startDate}</GlassChip>
//                                   <GlassChip theme={theme}>
...
//                                   <KV k="Price / Pax" v={String(overview.pricePerPax)} theme={theme} />
//                                   <KV k="Total Pax" v={String(overview.totalPax)} theme={theme} />
//                                 </div>
...
//                                   <div className="text-xs text-slate-500">
//                                     {dayText?.dateText ? `Date: ${dayText.dateText}` : ""}
//                                   </div>
//                                   <GlassChip theme={theme}>
...
//                                         <KV
//                                           k="Trip Vehicles"
//                                           v={seg.tripVehicles?.length ? seg.tripVehicles.join(", ") : "-"}
//                                           theme={theme}
//                                         />
//                                         <KV
//                                           k="Add-on Vehicles"
//                                           v={seg.addonVehicles?.length ? seg.addonVehicles.join(", ") : "-"}
//                                           theme={theme}
//                                         />
//                                         <KV
//                                           k="Food"
//                                           v={seg.foods?.length ? seg.foods.join(", ") : "-"}
//                                           theme={theme}
//                                         />
//                                         <KV
//                                           k="Accommodation"
//                                           v={seg.stays?.length ? seg.stays.join(", ") : "-"}
//                                           theme={theme}
//                                         />
//                                       </div>
//                                     </div>
...
//                         "
//                         style={{ borderColor: `${theme}66`, color: theme, background: "rgba(255,255,255,0.65)" }}
//                       >
...
function SoftInput({ value, onChange, theme, placeholder, type = "text", min = 0 }) {
  return (
    <input
      type={type}
      min={min}
...
/* -----------------------------
  Point & Discount Modal (UPDATED: smaller, no equations/details, no suggestions)
------------------------------ */
                ...
                <>
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <GlassChip theme={theme}>
                        Max: <span className="ml-1 font-mono">{money(maxDiscount)}</span>
                      </GlassChip>
                    </div>
                    ...
                    <div className="mt-3">
                      <SoftInput
                        ...
                        onChange={(e) => {
                          ...
                          const num = asNum(v);
                          setDiscountAmount(String(clamp(num, 0, maxDiscount || 0)));
                        }}
                        placeholder="0"
                      />
...
export default function GroupTourFullDetail({
  ...
  const dayText = useMemo(() => {
    ...
    const segmentBlocks = segments.map((seg, idx) => {
      ...
      const tripVehicles = Array.isArray(seg?.boTripVehicles)
        ? seg.boTripVehicles.map((v) => v?.vehicleName || v?.category).filter(Boolean)
        : [];

      const addonVehicles = Array.isArray(seg?.boAddonVehicles)
        ? seg.boAddonVehicles.map((v) => v?.vehicleName || v?.category).filter(Boolean)
        : [];
  ...
  const openPointDiscount = async () => {
    ...
    try {
      ...
      const res = await API.get("/executive/group-tour-point-discount-options", {
        params: { clientId: tour.clientId, groupTourId: tour.id },
      });
  ...
  /* -----------------------------
    Existing API actions (send discountAmount + executivePoint)
  ------------------------------ */
  // const handleReferralDownload = async () => {
  //   if (!nextDate || !nextTime) {
...
const handleReferralDownload = async () => {
  ...
  try {
    ...
    const response = await API.post(
      "/executive/group-tour-referral-itinerary",
      {
        clientId: tour.clientId,
        groupTourId: tour.id,
        groupTourName: tourFull?.tourName || tour?.name,
    ...
    a.href = url;
    a.download = `Referral-Itinerary-${tourFull?.tourName || "GroupTour"}.pdf`;
    ...
    toast.success("Referral itinerary downloaded successfully");
    if (typeof onCompleted === "function") onCompleted();
    else if (typeof onClose === "function") onClose();
  } catch (err) {
    const msg =
      err?.response?.data?.message ||
      err.message ||
      "Failed to download referral itinerary";
    toast.error(msg);
  } finally {
    setLoadingReferral(false);
  }
  ...
  // ✅ shared button style (Confirm-style) + reduced width
                                ...
                                <>
                                  ...
                                  {dayText?.segmentBlocks?.map((seg) => (
                                    ...
                                    >
                                      ...
                                      <div className="mt-4 space-y-4 text-sm text-slate-700">
                                        {seg.tripName && <KV k="Trip" v={seg.tripName} theme={theme} />}
                                        {seg.addonName && (
                                          <KV k="Add-on Trip" v={seg.addonName} theme={theme} />
                                        )}
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                          ...
                                          <KV
                                            k="Add-on Vehicles"
                                            v={
                                              seg.addonVehicles?.length
                                                ? seg.addonVehicles.join(", ")
                                                : "-"
                                            }
                                            theme={theme}
                                          />
