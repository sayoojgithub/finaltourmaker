// GroupTourFullDetail.jsx
import React, { useState } from "react";
import { motion, AnimatePresence } from "framer-motion";
import { X, CalendarClock, MapPin, Users, CircleDot } from "lucide-react";
import API from "../../api";
import { toast } from "react-toastify";

export default function GroupTourFullDetail({ tour, brandColor, onClose, onCompleted }) {
  if (!tour) return null;
  ...
  const handleReferralDownload = async () => {
    ...
    try {
      ...
      await API.post("/executive/group-tour-referral-itinerary", {
        ...
        groupTourId: tour.id,
        groupTourName: tour.name,
        nextDateRaw: nextDate,
        nextTimeRaw: nextTime,
      });
      toast.success("Referral itinerary status saved");
      // 🔮 later you can trigger actual file download from this API
      // 🔁 refresh SalesClients + ClientCategories and close overlays
  ...
  const handleConfirmDownload = async () => {
    try {
      ...
      await API.post("/executive/group-tour-confirm-itinerary", {
        ...
        groupTourId: tour.id,
        groupTourName: tour.name,
        nextDateRaw: nextDate || null, // optional
        nextTimeRaw: nextTime || null, // optional
      });
      toast.success("Confirmed itinerary status saved");
      // 🔮 later you can trigger actual file download from this API
      // 🔁 same refresh + close behavior
    if (typeof onCompleted === "function") {
      onCompleted();
    } else if (typeof onClose === "function") {
      onClose();
        ...
        >
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50/60 p-4 space-y-3">
            ...
            <div className="text-[11px] text-slate-500">
              • For <span className="font-semibold">Referral Itinerary</span>,
              date & time are <span className="font-semibold">mandatory</span>.
              <br />
              • For <span className="font-semibold">Confirmed Itinerary</span>,
              they are optional.
            </div>
          </div>
