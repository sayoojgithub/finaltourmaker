...
export default function ClientDetailsPanel({
  ...
  const handleSave = () => {
    ...
    const payload = {
      ...
      // destinations (PRIMARY DISABLED but still saved as existing value)
      primaryDestinationName: primaryDestination
        ? {
            _id: primaryDestination._id,
            value: primaryDestination.value,
      ...
      // addon destinations moved section
      addonDestinations: addonDestinations.map((d) => ({
        _id: d._id,
        value: d.value,
        label: d.label,
      ...
      // dates (DISABLED but still saved as existing value)
      startDate: startDateInput || null,
      endDate: endDateInput || null,
      numberOfDays:
        typeof numberOfDays === "number"
...
`}</style>
    ...
    <div className="h-full flex flex-col">
      ...
      {/* Body */}
      <div className="p-4 sm:p-5 space-y-4 overflow-y-auto scroll-smooth max-h-[420px] ct-glass-scroll">
        {/* Contact & location */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {/* Contact editable */}
          <div className="rounded-2xl border border-slate-200 bg-slate-50/60 p-3.5">
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50/60 p-3.5">
            ...
            <div className="space-y-1.5 text-xs text-slate-600">
              ...
              <div className="flex flex-col gap-0.5">
                <span className="text-[11px] text-slate-500">Contact via</span>
                <Select
                  options={contactOptions}
                  value={clientContactOption}
                  onChange={setClientContactOption}
        ...
        {/* Destinations + group */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {/* Destinations (PRIMARY disabled, addon moved out) */}
          <div className="rounded-2xl border border-slate-200 bg-slate-50/60 p-3.5">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <div className="h-7 w-7 rounded-xl bg-white flex items-center justify-center border border-slate-200 text-slate-600">
            ...
            <div className="space-y-2 text-xs">
              <div>
                ...
                {/* Disabled select with same style */}
                <Select
                  isLoading={loadingDest}
                  options={destinations}
                  value={primaryDestination}
                  onChange={() => {}}
                  placeholder={loadingDest ? "Loading..." : "Primary destination"}
                  styles={{
                    ...selectStyles,
                    control: (base, state) => ({
                ...
                <div className="mt-1 text-[11px] text-slate-400">
                  Primary destination is locked here (change via Contact Flow → Change → Destination).
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50/60 p-3.5">
            ...
            <div className="space-y-2 text-xs text-slate-600">
              <div className="flex flex-col gap-0.5">
                <span className="text-[11px] text-slate-500">Group type</span>
                <Select
                  options={groupTypeOptions}
                  value={groupType}
                  onChange={setGroupType}
              ...
              <div className="flex flex-col gap-0.5">
                <span className="text-[11px] text-slate-500">No. of persons</span>
                <input
                  type="number"
                  min={1}
                  value={persons}
          ...
          <div className="grid grid-cols-2 gap-3 text-xs">
            <div>
              ...
              <input
                ...
                onChange={() => {}}
                disabled
                className="w-full rounded-xl border border-slate-300 bg-slate-100 px-2 py-1.5 text-[12px] outline-none cursor-not-allowed opacity-80"
              />
            </div>
            <div>
              <div className="text-[11px] text-slate-500 mb-1">End</div>
            ...
            <div className="col-span-2">
              <div className="text-[11px] text-slate-500 mb-0.5">Number of days</div>
              <div className="text-[12px] font-medium text-slate-800">
                {numberOfDays !== "-" ? `${numberOfDays} days` : "-"}
              </div>
              <div className="mt-1 text-[11px] text-slate-400">
                Dates are locked here (change via Contact Flow → Change → Date).
              </div>
            </div>
          </div>
        </div>

        {/* Behaviour & source – read-only + GST editable */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <InfoCard
            title="Client Insight"
            icon={CircleDot}
            items={[
              { label: "Behavior", value: client.behavior?.label },
              { label: "Connected through", value: client.connectedThrough?.label },
            ]}
          />
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50/60 p-3.5">
            ...
            <div className="space-y-1.5 text-xs text-slate-600">
              <div className="flex flex-col gap-0.5">
                <span className="text-[11px] text-slate-500">GST Number</span>
                <input
                  type="text"
                  value={gstNumber}
                  onChange={(e) => setGstNumber(e.target.value)}
        ...
        {/* ✅ Add-on destinations moved here (same pattern as additional requirements) */}
        <div>
          <div className="text-xs font-semibold text-slate-500 uppercase tracking-[0.14em] mb-1">
            Add-on destinations
          </div>
          ...
          {addonDestinations.length ? (
            ...
            <div className="text-xs text-slate-400">No add-on destinations</div>
          )}
        </div>
