// src/pages/creative/UploadRequests.jsx
import React, { useEffect, useMemo, useState } from "react";
import API from "../../api";
import Select from "react-select";
import { toast } from "react-toastify";

export default function CreativeUploadRequests() {
  // table
  ...
  // filename rows (user-controlled add/remove)
...
const isFinalized = (s) => s === "approved" || s === "rejected";
  ...
  const fetchList = async (nextPage = page) => {
    try {
      setLoading(true);
      const res = await API.get(`/creativeStaff/upload-requests?${buildQuery(nextPage)}`);
  ...
  const openPanel = async (id) => {
    ...
    setFileNames([]);
    try {
      const res = await API.get(`/creativeStaff/upload-requests/${id}`);
      const d = res.data;
      setDetail(d);
  ...
  const flipToggled = async (id) => {
    try {
      setTogglingId(id);
      const res = await API.post(`/creativeStaff/upload-requests/${id}/toggle`);
  ...
  const saveNames = async () => {
    ...
    try {
      await API.post(`/creativeStaff/upload-requests/${detail._id}/filenames`, {
  ...
  const sendForApproval = async () => {
    if (!detail?._id) return;
    try {
      await API.post(`/creativeStaff/upload-requests/${detail._id}/send-for-approval`);
  ...
  return (
    <div className="space-y-6">
      <h2 className="text-xl font-semibold text-[#222]">Upload Requests — Creative Staff</h2>
          ...
          {panelLoading || !detail ? (
            ...
            <>
              ...
              {/* <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                <Field label="Deadline (Approved or Original)">
                  <input
                    readOnly
                    className="w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2"
                    value={formatDMY(detail.approvedPublishingDate || detail.publishingDate)}
                ...
                <Field label="Creative Status">
                  <input
                    ...
                    value={detail.creativeStatus || "pending"}
                  />
                </Field>
              </div> */}
